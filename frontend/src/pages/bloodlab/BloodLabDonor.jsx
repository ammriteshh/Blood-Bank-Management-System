...
const BloodLabDonor = () => {
  ...
  const searchDonors = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_ENDPOINTS.BLOOD_LAB}/donors/search?term=${term}`,
  ...
  const loadRecentDonations = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(
        `${API_ENDPOINTS.BLOOD_LAB}/donations/recent`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const markDonation = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${API_ENDPOINTS.BLOOD_LAB}/donors/donate/${selectedDonor._id}`,
        donationData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const quickDonation = async (donorId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${API_ENDPOINTS.BLOOD_LAB}/donors/donate/${donorId}`,
